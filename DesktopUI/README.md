...
> **ZGALAXY fork:** this copy is adapted for ZGALAXY One. The crate is named
> `zgalaxy_desktop_ui` (v1.16.2) and builds `zgalaxy_desktop_ui.exe` /
> `zerotier_desktop_ui.exe`. It is built automatically by
> `windows/build-windows.ps1` and bundled into `ZGALAXY-One-Setup.exe`.
